...
app.post("/signup", async (req, res) => {
  try {
    ...
    username = username.toLowerCase(); // Ensure case-insensitive usernames
...
app.post("/auction", authenticate, async (req, res) => {
  try {
    ...
    const newItem = new AuctionItem({
      itemName,
      description,
      currentBid: Number(startingBid), // Ensure numeric bid value
      highestBidder: "",
      closingTime,
    });
...
// ✅ Update an auction item (Protected)
app.put("/auctions/:id", authenticate, async (req, res) => {
  try {
    ...
    const updatedAuction = await AuctionItem.findByIdAndUpdate(
      id,
      { itemName, description, currentBid: Number(startingBid), closingTime },
      { new: true }
    );
